...
def read_test_data():
    ...
    if IMAGE_SIZE == 128:
        filename_test = tf.train.string_input_producer(["TFRecord128_2/test.tfrecords"])
    else:
        filename_test = tf.train.string_input_producer(["TFRecord64_1/test.tfrecords"])
...
def train():
    ...
    x_batch_test, y_batch_test = tf.train.batch([x_test, y_test], batch_size=BATCH_SIZE, capacity=200, num_threads=3)

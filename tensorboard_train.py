...
def net(image, drop_pro):
    ...
    fc_2 = weights_with_loss([1024, 8], stddev=0.01, wl=0.0)
    fc_b2 = biasses([8])
...
def read_train_data():
    ...
    filename_train = tf.train.string_input_producer(["TFRecord128_4/train.tfrecords"])
    ...
    labels_train = tf.one_hot(labels_train, 8)
...
def read_test_data():
    reader = tf.TFRecordReader()
    filename_test = tf.train.string_input_producer(["TFRecord128_4/test.tfrecords"])
    ...
    labels_test = tf.one_hot(labels_test, 8)
    return images_test, labels_test

def save_model(sess, step):
    MODEL_SAVE_PATH = "./model128_4/"
...
def train():
    ...
    with tf.name_scope('getAccuracy'):
        ...
        getAccuracy_summary = scalar_summary('getAccuracy', getAccuracy)
    with tf.name_scope('cross_entropy'):
        cross_entropy = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(logits=logits, labels=y))
        cross_entropy_summary = scalar_summary('cross_entropy', cross_entropy)
    ...
    for i in range(10):
        ...
        if i % 1 == 0:
            ...
            result = sess.run(merged, feed_dict={x: _images_train, y: label_train, drop_pro: 1})  # merged也是需要run的
